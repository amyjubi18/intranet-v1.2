    ...
    <body class="font-sans antialiased text-gray-900 ">
        <section class="flex flex-col items-center h-screen border-4 border-blue-900 rounded md:flex-row ">
            ...
            <div class="w-full px-6 py-4 mt-6 overflow-hidden bg-white sm:max-w-md sm:rounded-lg">
                ...
                <h1 class="py-3 pb-0 text-3xl font-bold text-center text-blue-900">Registrar una Subcategoría</h1>

                <form action="{{route('administracion.sub_categoria.store')}}" method="POST" enctype="multipart/form-data">
                    @csrf


                    <div>
                        <x-input-label for="sub_categoria" :value="__('Subcategoría:')" />
                        <x-text-input id="sub_categoria" class="block w-full mt-1" type="text" name="sub_categoria" :value="old('sub_categoria')" required autofocus />
                        <x-input-error :messages="$errors->get('sub_categoria')" class="mt-2" />
                    </div>
                    <div class="flex mt-4 justify-evenly">

                        <a class="inline-flex items-center px-4 py-2 text-xs font-semibold tracking-widest text-white uppercase transition duration-150 ease-in-out bg-blue-900 border border-transparent rounded-md hover:bg-blue-500 focus:bg-blue-500 active:bg-blue-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 " href="{{ route('administracion.sub_categoria.index') }}">
                        ...
                        <x-primary-button class="ml-4" onsubmit="return route('administracion.sub_categoria.index')">
