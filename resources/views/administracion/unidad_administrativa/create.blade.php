    ...
    <body class="font-sans antialiased text-gray-900 ">
        <section class="flex flex-col items-center h-screen border-4 border-blue-900 rounded md:flex-row ">
            ...
            <div class="w-full px-6 py-4 mt-6 overflow-hidden bg-white sm:max-w-md sm:rounded-lg">
                ...
                <h1 class="py-3 pb-0 text-3xl font-bold text-center text-blue-900">Registrar la Unidad Administrativa</h1>

                <form action="{{route('administracion.unidad_administrativa.store')}}" method="POST" enctype="multipart/form-data">
                    @csrf


                    <div>
                        <x-input-label for="unidad_administrativa" :value="__('Unidad Administrativa:')" />
                        <x-text-input id="unidad_administrativa" class="block w-full mt-1" type="text" name="unidad_administrativa" :value="old('unidad_administrativa')" required autofocus  />
                        <x-input-error :messages="$errors->get('unidad_administrativa')" class="mt-2" />
                    </div>
                    <div class="flex mt-4 justify-evenly">

                        <a class="inline-flex items-center px-4 py-2 text-xs font-semibold tracking-widest text-white uppercase transition duration-150 ease-in-out bg-blue-900 border border-transparent rounded-md hover:bg-blue-500 focus:bg-blue-500 active:bg-blue-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 " href="{{ route('administracion.unidad_administrativa.index') }}">
                        ...
                        <x-primary-button class="ml-4" onsubmit="return route('administracion.unidad_administrativa.index')">
