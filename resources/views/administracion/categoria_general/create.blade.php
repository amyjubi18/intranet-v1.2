    ...
    <body class="font-sans antialiased text-gray-900 ">
        <section class="flex flex-col items-center h-screen border-4 border-blue-900 rounded md:flex-row ">
            ...
            <div class="w-full px-6 py-4 mt-6 overflow-hidden bg-white sm:max-w-md sm:rounded-lg">
                ...
                <h1 class="py-3 pb-0 text-3xl font-bold text-center text-blue-900">Registrar una Categoría General</h1>

                <form action="{{route('administracion.categoria_general.store')}}" method="POST" enctype="multipart/form-data">
                    @csrf


                    <div>
                        <x-input-label for="categoria_general" :value="__('Categoría General:')" />
                        <x-text-input id="categoria_general" class="block w-full mt-1" type="text" name="categoria_general" :value="old('categoria_general')" required autofocus />
                        <x-input-error :messages="$errors->get('categoria_general')" class="mt-2" />
                    </div>
                    <div class="flex mt-4 justify-evenly">
                        ...
                        <x-primary-button class="ml-4" onsubmit="return route('administracion.categoria_general.index')">
