...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{'Editar Forma de Adquisición' }}</title>
    ...
    <form method="POST" action="{{ route('administracion.forma_adquisicion.update', $forma_adquisicion->id)}}">
        @csrf
        @method('PUT')
        <div>
            <x-input-label for="forma_adquisicion" :value="__('Forma de Adquisición:')" />
            <x-text-input id="forma_adquisicion" class="block w-full mt-1" type="text" name="forma_adquisicion" value="{{old('forma_adquisicion', $forma_adquisicion->forma_adquisicion)}}" autofocus />
            <x-input-error :messages="$errors->has('forma_adquisicion')" class="mt-2" />
        </div>

        <div class="flex mt-4 justify-evenly">

            <a class="inline-flex items-center px-4 py-2 text-xs font-semibold tracking-widest text-white uppercase transition duration-150 ease-in-out bg-blue-900 border border-transparent rounded-md hover:bg-blue-500 focus:bg-blue-500 active:bg-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 " href="{{ route('administracion.forma_adquisicion.index') }}">
