    ...
    <body class="font-sans antialiased text-gray-900 ">
        <section class="flex flex-col items-center h-screen border-4 border-blue-900 rounded md:flex-row ">
            ...
            <div class="w-full px-6 py-4 mt-6 overflow-hidden bg-white sm:max-w-md sm:rounded-lg">
                <h1 class="my-1 text-5xl font-bold text-center text-blue-900" style="padding-top: -10px">Editar Categoría</h1>


    <form method="POST" action="{{ route('administracion.categorias.update', $categoria->id)}}">
        @csrf
        @method('PUT')
        <div>
            <x-input-label for="name" :value="__('Categoría:')" />
            <x-text-input id="name" class="block w-full mt-1" type="text" name="name" value="{{old('name', $categoria->name)}}" autofocus />
            <x-input-error :messages="$errors->has('name')" class="mt-2" />
        </div>

        <div class="flex mt-4 justify-evenly">

            <a class="inline-flex items-center px-4 py-2 text-xs font-semibold tracking-widest text-white uppercase transition duration-150 ease-in-out bg-blue-900 border border-transparent rounded-md hover:bg-blue-500 focus:bg-blue-500 active:bg-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 " href="{{ route('administracion.categorias.index') }}">
