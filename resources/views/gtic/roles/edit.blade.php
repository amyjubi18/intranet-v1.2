    ...
    <body class="font-sans antialiased text-gray-900 ">
        <section class="flex flex-col items-center h-full border-4 border-blue-900 rounded md:flex-row ">
            ...
            <div class="w-full px-6 py-4 mt-6 overflow-hidden bg-white sm:max-w-md sm:rounded-lg">
                <h1 class="py-3 pb-0 text-3xl font-bold text-center text-blue-900">Editar Rol</h1>
    <form method="POST" action="{{ route('gtic.roles.update', $role->id)}}">
        @csrf
        @method('PUT')
        <div>
            ...
            <tr>
                <td>
                    <div>
                        ...
                        <label>
                            <input type="checkbox" name="permissions[]" value="{{$id}}"{{$role->permissions->contains($id) ? 'checked': ''}} id="">
                            <span>
                                <span></span>
                            </span>
                        </label>
        ...
        <div class="flex justify-evenly mt-4">
            ...
            <a class="inline-flex items-center px-4 py-2 bg-blue-900 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-500 focus:bg-blue-500 active:bg-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition ease-in-out duration-150 " href="{{ route('gtic.roles.index') }}">
